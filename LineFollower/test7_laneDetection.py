...
video_path = 'footage/mp4TestLaneDetection4.mp4'
...
while (True):
    ...
    fliped_image = cv2.flip(vertical_flip, 0)
    
    mask = np.zeros(fliped_image.shape[:2], dtype="uint8")
    #pts = [(440, 0), (300, 200), (700, 200), (840, 0)]
    pts = [(640, 720), (690, 520), (840, 520),(890, 720)]
    cv2.fillPoly(mask, np.array([pts]), 255)
    #cv2.rectangle(mask, (400,420), (880, 720), 255, -1)
    crop = cv2.bitwise_and(fliped_image, fliped_image, mask=mask)
    #crop = fliped_image[500:720, 400:1020]
    ...
    gray_raw_image = cv2.cvtColor(brightImage, cv2.COLOR_BGR2GRAY)
    ...
    blurred = cv2.blur(gray_raw_image, (3,5))
    ...
    #lowThresh = ret*0.5
    #print(highThresh)
    #dst = cv2.Canny(thresh, float(lowThresh), float(highThresh), None, 3)
    dst = cv2.Canny(thresh, 60, 120, None, 5)
